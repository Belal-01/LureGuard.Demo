  ...
  return (
    <button
      ...
      className={`inline-flex items-center gap-2 px-3 py-1.5 rounded-md text-xs font-mono transition-all duration-300 border ${
        enabled
          ? 'bg-slate-900/80 border-emerald-500/40 text-emerald-400 hover:border-emerald-400 hover:shadow-[0_0_12px_rgba(52,211,153,0.2)]'
    ...
    >
      ...
      <Activity className={`w-3.5 h-3.5 ${enabled ? 'text-emerald-400 animate-pulse' : 'text-slate-500'}`} />
    </button>
  );
};
